...
document.addEventListener('DOMContentLoaded', () => {
    const page = window.location.pathname.split('/').pop(); // e.g., 'rentals.html', 'airbnb.html', 'land.html'

let pageType = 'all';
if (page === 'rentals.html') pageType = 'rent';
  ...
  function init() {
    ...
    const page = window.location.pathname.split('/').pop(); // e.g., 'rentals.html', 'airbnb.html'
    let pageType = 'all';
    if (page === 'rentals.html') pageType = 'rent';
    else if (page === 'forsale.html') pageType = 'sale';
    else if (page === 'land.html') pageType = 'land';
    ...
    // ========== CUSTOMIZE FILTER UI BASED ON PAGE TYPE ==========
    customizeFiltersForPage(pageType);
    
    // ========== SET ACTIVE TRANSACTION BASED ON PAGE TYPE ==========
    if (pageType === 'rent') {
        state.activeTransaction = 'rent';
        // Activate the correct tab button (if exists)
        const rentTab = document.querySelector('.tab-btn[data-type="rent"]');
        if (rentTab) rentTab.click();
    } else if (pageType === 'sale') {
        state.activeTransaction = 'sale';
        const saleTab = document.querySelector('.tab-btn[data-type="sale"]');
        if (saleTab) saleTab.click();
    } else if (pageType === 'land') {
        state.activeTransaction = 'sale'; // land is usually for sale
        // Force property type filter to land types
        if (elements.typeFilter) elements.typeFilter.value = 'land-res';
        const saleTab = document.querySelector('.tab-btn[data-type="sale"]');
        if (saleTab) saleTab.click();
    } else if (pageType === 'airbnb') {
        state.activeTransaction = 'rent';
        const rentTab = document.querySelector('.tab-btn[data-type="rent"]');
        if (rentTab) rentTab.click();
    } else if (pageType === 'location') {
        // Locations page doesn't need property grid filters
        // You may skip loading properties entirely or load minimal data
        console.log('Locations page – skipping property grid load');
        return;
    }
...
// Helper function to customize filter controls per page
function customizeFiltersForPage(pageType) {
    const sizeGroup = document.getElementById('size-container');
    const bedroomsGroup = document.getElementById('bedrooms-container');
    const priceSelect = document.getElementById('price-range');
    
    // First, show/hide common groups
    if (pageType === 'land') {
        ...
        if (bedroomsGroup) bedroomsGroup.style.display = 'flex';
        if (sizeGroup) sizeGroup.style.display = 'none';
        if (priceSelect) {
            priceSelect.innerHTML = `
                <option value="all">Any Price</option>
                <option value="0-3k">Under KES 3k / night</option>
                <option value="3k-6k">KES 3k – 6k / night</option>
                <option value="6k-10k">KES 6k – 10k / night</option>
                <option value="over-10k">Over KES 10k / night</option>
            `;
        }
        ...
        // default / all properties page – show both bedrooms and size? your call
        if (bedroomsGroup) bedroomsGroup.style.display = 'flex';
        if (sizeGroup) sizeGroup.style.display = 'none';
        // keep default price options
    }
}
...
function detectPeriod(rawPrice = '') {
  ...
  function applyFilters(initialLoad = false) {
    ...
    state.filteredProperties = state.properties.filter(property => {
        // 0. PAGE‑TYPE FILTER (overrides everything)
        if (state.pageType === 'land') {
            // Only land types (residential, commercial, agricultural)
            const landTypes = ['land-res', 'land-comm', 'ranch'];
            if (!landTypes.includes(property.type)) {
                console.log('❌ Failed land‑type filter');
                return false;
            }
        } else if (state.pageType === 'airbnb') {
            // Only properties marked as short‑stay / furnished (if you have a flag)
            // For now, we filter by transaction=rent AND type in typical Airbnb types
            if (property.transaction !== 'rent') return false;
            // Optional: further restrict to furnished/apartment/villa
            const airbnbTypes = ['apartment', 'villa', 'studio', 'furnished'];
            if (!airbnbTypes.includes(property.type)) {
                console.log('❌ Failed Airbnb type filter');
                return false;
            }
        } else if (state.pageType === 'rent') {
            // Already enforced by transaction filter, but double‑check
            if (property.transaction !== 'rent') return false;
        } else if (state.pageType === 'sale') {
            if (property.transaction !== 'sale') return false;
        }
        ...
        // 4. Type filter (if not already overridden by pageType)
...
// Add this inside your DOMContentLoaded or after properties are loaded
function handleUrlFilters() {
    const urlParams = new URLSearchParams(window.location.search);
    
    // Handle type parameter (rent, sale, plots)
    ...
    if (locationParam && elements.locationFilter) {
        elements.locationFilter.value = locationParam;
    }
    
    // Handle subtype parameter
    const subtypeParam = urlParams.get('subtype');
    if (subtypeParam && elements.typeFilter) {
        elements.typeFilter.value = subtypeParam;
    }
    
    // Handle search parameter
    const searchParam = urlParams.get('search');
    if (searchParam && elements.filterSearch) {
        elements.filterSearch.value = searchParam;
        state.filters.search = searchParam.toLowerCase();
    }
    
    // Apply filters if any parameters were set
    if (typeParam || locationParam || subtypeParam || searchParam) {
        setTimeout(() => applyFilters(), 100);
    }
}

// Call this after properties are loaded
// Inside your loadProperties success callback, add:
// handleUrlFilters();